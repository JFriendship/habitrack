...
def mark_habit_complete(
    db: Session, habit_id: int, user_id: int, completed_date: date | None = None
):
    _get_habit_or_raise(db=db, habit_id=habit_id, user_id=user_id)

    completed_date = completed_date or datetime.now(UTC).date()

    existing_completion = get_completion_for_habit_and_date(
        db=db, habit_id=habit_id, completed_date=completed_date
    )
...
def unmark_habit_complete(
    db: Session, habit_id: int, user_id: int, completed_date: date
):
    # Make sure user owns the habit and that the habit exists
    _get_habit_or_raise(db=db, habit_id=habit_id, user_id=user_id)

    completion = get_completion_for_habit_and_date(
        db=db, habit_id=habit_id, completed_date=completed_date
    )
...
def habit_completed_on_date(
    db: Session, habit_id: int, user_id: int, completed_date: date
) -> bool:
    _get_habit_or_raise(db=db, habit_id=habit_id, user_id=user_id)

    return completion_exists_for_habit_and_date(
        db=db, habit_id=habit_id, completed_date=completed_date
    )
