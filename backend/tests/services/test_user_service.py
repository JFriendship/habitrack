...
def test_register_user_works(db_session):
    user_create = UserCreate(
        username="JohnDoe42", email="johndoe42@gmail.com", password="johnDaBest4242"
    )
...
def test_register_user_breaks(db_session):
    ...
    with pytest.raises(EmailAlreadyExistsError):
        user_create = UserCreate(
            username="JohnathanDoe", email="johndoe42@gmail.com", password="johnieBest"
        )
        user = register_user(db=db_session, user_create=user_create)

    with pytest.raises(UsernameAlreadyExistsError):
        user_create = UserCreate(
            username="JohnDoe42", email="johnieboy42@gmail.com", password="johnDaBest42"
        )
        user = register_user(db=db_session, user_create=user_create)
...
def test_authenticate_user_works(db_session):
    ...
    user_create = UserCreate(username="JohnDoe42", email=email, password=password)
    user = register_user(db=db_session, user_create=user_create)
