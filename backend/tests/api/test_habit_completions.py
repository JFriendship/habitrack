...
def test_complete_habit(client, auth_headers, created_habit):
    response = client.post(
        f"/api/habits/{created_habit.id}/complete", headers=auth_headers
    )
...
def test_habits_completion_history(client, auth_headers, created_habit):
    ...
    response = client.get(
        f"/api/habits/{created_habit.id}/completions", headers=auth_headers
    )
...
def test_completion_status(client, auth_headers, created_habit):
    ...
    response = client.get(
        f"/api/habits/{created_habit.id}/completed-on/{today}", headers=auth_headers
    )
...
def test_habit_completion_deletion(client, auth_headers, created_habit):
    ...
    response = client.delete(
        f"/api/habits/{created_habit.id}/complete/{today}", headers=auth_headers
    )
